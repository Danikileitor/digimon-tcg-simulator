...
const useTabs = _ref => {
  ...
  const values = useMemo(() => tabs.reduce((all, tab) => {
  ...
  const getTabListProps = useCallback(function (_temp) {
    let {
      role = 'tablist',
      ...other
    } = _temp === void 0 ? {} : _temp;
  ...
  const getTabProps = useCallback(_ref2 => {
    let {
      role = 'tab',
      value,
      ...other
  ...
  const getTabPanelProps = useCallback(_ref3 => {
    let {
      role = 'tabpanel',
      value,
      ...other
