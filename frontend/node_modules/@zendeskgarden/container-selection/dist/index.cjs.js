...
const useSelection = _ref => {
  ...
  const refs = React.useMemo(() => values.reduce((all, value) => {
  ...
  const controlledFocusedValue = containerUtilities.getControlledValue(focusedValue, state.focusedValue);
  const controlledSelectedValue = containerUtilities.getControlledValue(selectedValue, state.selectedValue);
  React.useEffect(() => {
  ...
  const getElementProps = _ref2 => {
    ...
    const handleKeyDown = event => {
      ...
      if (!hasModifierKeyPressed) {
        if (event.key === containerUtilities.KEYS.UP && verticalDirection || event.key === containerUtilities.KEYS.LEFT && horizontalDirection) {
        ...
        } else if (event.key === containerUtilities.KEYS.DOWN && verticalDirection || event.key === containerUtilities.KEYS.RIGHT && horizontalDirection) {
          if (rtl && horizontalDirection) {
            onDecrement();
          } else {
            onIncrement();
          ...
          event.preventDefault();
        } else if (event.key === containerUtilities.KEYS.HOME) {
          ...
          event.preventDefault();
        } else if (event.key === containerUtilities.KEYS.END) {
          ...
          event.preventDefault();
        } else if (event.key === containerUtilities.KEYS.SPACE || event.key === containerUtilities.KEYS.ENTER) {
          onSelect && onSelect(value);
          !isSelectedValueControlled && dispatch({
            type: 'KEYBOARD_SELECT',
            payload: value
